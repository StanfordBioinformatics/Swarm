...
class AzureBatchScheduler(BaseBatchSchduler):
    ...
    def _get_azure_batch_client(conf):
        ...
        creds = batch_auth.SharedKeyCredentials(conf[utils.PLATFORM]['batch_account'], conf[utils.PLATFORM]['batch_key'])
    ...
    def _get_azure_container_client(conf):
        ...
        client = BlobServiceClient.from_connection_string(conf[utils.PLATFORM]['storage_connection_string'])
        container_client = client.get_container_client(container=conf[utils.PLATFORM]['storage_container'])
