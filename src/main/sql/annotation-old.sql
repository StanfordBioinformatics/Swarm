SELECT
  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        alternate_bases
      FROM
        `gbsc-gcp-project-annohive-dev.AnnotationHive.NA12877_chr17_Lek`) ) AS VCF
  JOIN
    `gbsc-gcp-project-annohive-dev.AnnotationHive.hg19_Variant_9B_Table` AS Annotation1
  ON
  ...
  WHERE
    ((
        ...
        AND
        (CONCAT(VCF.reference_bases, Annotation1.alternate_bases) = VCF.alternate_bases)
